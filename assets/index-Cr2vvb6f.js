...
`).map(Ub).filter(y=>y!==null),r=t.filter(y=>y.type===zo.BODY_LINE).map(y=>y.body),s=r.findIndex(y=>y.lyrics||y.chords.length),c=r.length-r.slice().reverse().findIndex(y=>y.lyrics||y.chords.length),d=r.slice(s,c),f=t.filter(y=>y.type===zo.DIRECTIVE).reduce((y,w)=>({...y,[w.directive]:w.value}),{});return f.title===void 0||f.number===void 0?null:{title:f.title,number:f.number,latinTitle:nz(f.title||""),body:d}},Zb=a=>{let t=0;for(let r=0;r<a.length;r++)t=(t<<5)-t+a.charCodeAt(r),t=t&65535;return t.toString(16).toUpperCase().padStart(4,"0")},Vb=a=>({songs:a.split(/---\s*\n/).map($b).filter(r=>r!==null),checksum:Zb(a)}),Yb=()=>{const a=Vb(Hb);return x.jsx(Lb,{parsedSongbook:a})};var to={},Gf;function Qb(){if(Gf)return to;Gf=1;var a=Nf();return to.createRoot=a.createRoot,to.hydrateRoot=a.hydrateRoot,to}var Xb=Qb();const qb=Xb.createRoot(document.getElementById("root"));qb.render(x.jsx(zn.StrictMode,{children:x.jsx(Yb,{})}));
